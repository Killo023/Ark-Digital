...
export const clientStore = {
  getAll(): Client[] {
    ...
    stored.forEach((c) => {
      ...
      if (c.name === 'Shepherd Studios' && c.lastPaymentDate === null) {
        c.lastPaymentDate = '2025-01-21'
        changed = true
      }
      ...
      if (c.name === 'Chernelang Physio') {
        if (c.hostingFeeMonthly === 0) {
          c.hostingFeeMonthly = 100
          changed = true
        }
        if (c.lastPaymentDate === null) {
          c.lastPaymentDate = '2025-01-15'
          changed = true
        }
      }
